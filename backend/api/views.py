...
from api.serializers import (FavoritesSerializer, ShoppingCartSerializer, UserSerializer, AvatarSerializer,
                             SubscribeSerializer, SubscriptionsSerializer, RecipeSerializer, ShortRecipesSerializer,
                             TagSerializer,
...
class UserViewSet(DjoserViews):
    ...
    def subscribe(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
    ...
    def unsubscribe(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
        author = get_object_or_404(User, id=pk)
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def delete_obj(
        ...
        recipe = get_object_or_404(Recipe, id=pk)
        obj_deleted = model.objects.filter(user=request.user, recipe=recipe).delete()[0]
        if not obj_deleted:
            return Response(
    ...
    def favorite(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
        return self.create_obj(self.serializer_class, request, pk)

    @favorite.mapping.delete
    def del_favorite(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
    ...
    def shopping_cart(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
        return self.create_obj(self.serializer_class, request, pk)
    ...
    def short_link(self, request: HttpRequest, pk: Optional[int] = None) -> Response:
        recipe = get_object_or_404(Recipe, id=pk)
        if not recipe.short_code:
            recipe.short_code = recipe.generate_short_code()
            recipe.save()
        short_url = request.build_absolute_uri(f"/recipes/short/{recipe.short_code}/")
